...
const LoginForm: React.FC<LoginFormProps> = ({ onLogin }) => {
    ...
    const onSubmit = async (data: LoginFormData) => {
        try {
            ...
            if (sanitizedEmail === mockUserData.email && sanitizedPassword === mockUserData.password) {
    ...
    return (
        <div className="relative">
            <div className={`flex justify-center items-start mt-6 ${showModal ? 'blur-md' : ''}`}>
                <form
                    onSubmit={handleSubmit(onSubmit)}
                    className="w-[498px] h-[723px] border p-6 flex flex-col justify-center"
                >
                    <h1 className="mb-4 text-2xl font-bold text-center">덕업일치 계정으로 로그인해주세요.</h1>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="p-6 text-center bg-white rounded shadow-lg">
                        ...
                        <p>{errorMessage}</p>
                        <button onClick={closeModal} className="px-4 py-2 mt-4 text-white bg-gray-800 rounded">
                            닫기
                        </button>
                    </div>
                </div>
